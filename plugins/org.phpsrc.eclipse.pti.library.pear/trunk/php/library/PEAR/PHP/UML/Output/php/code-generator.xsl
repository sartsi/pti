...
<xsl:stylesheet version="1.0" exclude-result-prefixes="uml xmi php exslt exslt-set exslt-functions" 
	...
	<xsl:variable name="fileprefixDatatype"></xsl:variable>
	<xsl:variable name="fileprefixClass"></xsl:variable>
	<xsl:variable name="fileprefixInterface"></xsl:variable>
	...
	<xsl:template name="inner-container">
		...
		<xsl:call-template name="generateClassifierFiles">
			...
			<xsl:with-param name="setEntities" select="(packagedElement|nestedClassifier)[@xmi:type='uml:Interface' or @xmi:type='uml:Class']"/>
		</xsl:call-template>
	...
	<xsl:template name="generateClassifierFiles">
		...
		<xsl:for-each select="$setEntities">
			...
			<xsl:copy-of select="php:function($phpSaveToFile, string(concat(@name, '.php')), $content)" />
