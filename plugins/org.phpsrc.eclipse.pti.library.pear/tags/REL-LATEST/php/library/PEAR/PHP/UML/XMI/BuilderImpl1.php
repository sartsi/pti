...
class PHP_UML_XMI_BuilderImpl1 extends PHP_UML_XMI_AbstractBuilder
{
    ...
    {
        return '<UML:Package xmi.id="'.$package->id.'" name="'.$package->name.'">';
    }
    ...
    {
        return '<UML:DataType xmi.id="'.$type->id.'"'.
    ...
    static protected function getStructuralFeatureType(PHP_UML_Metamodel_TypedElement &$element)
    {
        ...
        } elseif (get_class($element->type)=='PHP_UML_Metamodel_Type') {
        ...
        if ($element->default!='') {
            $str .= '<UML:Attribute.initialValue>'.
                '<UML:Expression xmi.id="'.$id.'"'.
                ' body="'.htmlentities($element->default, ENT_QUOTES, 'UTF-8').'" />'.
